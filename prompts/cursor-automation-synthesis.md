...
Setup guide for [Cursor Automations](https://cursor.com/automations). **One automation** replaces three per-type automations. Runnable steps live in `prompts/synthesis-dispatcher-run.md`.
...
Optional backup crons (only if pre-fetch was missed that day):

| Type | Cron (UTC) |
|------|------------|
| News | `0 7 * * 1-5` |
| Berlin culture | `0 7 * * 2` |
| Berlin restaurants | `0 7 * * 4` |
